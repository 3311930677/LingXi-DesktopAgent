...
| 操作 | 行为 |
|---|---|
| `A`–`Z` | 累积全拼或纯简拼并刷新候选（如 `dang`、`nh`、`zgr`） |
...
引擎支持纯简拼：例如 `nh` 可召回“你好”，`zgr` 可召回“中国人”。当前暂不支持 `zhonggr`、`nhao` 这类全拼和简拼混输。

如果 `dang`、`zhong` 等常用字没有候选，先检查候选窗是否显示“大词库”。停止旧进程后重新执行 `scripts/run-dev.ps1`；该脚本会强制关闭旧版 `ime-server`/overlay，避免 9527 端口仍运行旧二进制。也可以单独运行：
